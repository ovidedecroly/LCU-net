...
def LCU_Net_with_C2f(input_shape=(256, 256, 3), output_shape=(256, 256, 1), debug=False):
    ...
    # Lanjutkan dengan sisanya, menyesuaikan c1 dan c2 sesuai

    # Output layer
    output_block = Conv2D(filters=output_shape[-1], kernel_size=1, strides=1, padding='same', activation='sigmoid')(down_sampled2)
